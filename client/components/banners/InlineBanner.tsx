/**
 * InlineBanner — Compact horizontal banner embedded in page content.
 *
 ...
 *
 * Desktop: max-height 120px, full-width, rounded
 * Mobile: max-height 100px, full-width
 */
...
async function trackEvent(bannerId: string, event: "view" | "click"): Promise<void> {
  try {
    ...
    await fetch(endpoint, {
      method: "POST",
      headers,
      body: JSON.stringify({ session_id: sessionStorage.getItem("banner_session_id") || "anonymous" }),
    });
  } catch {
    // best-effort
  }
  ...
  const content = (
    <div className={`w-full overflow-hidden rounded-xl ${className ?? ""}`}>
      <img
        src={imageUrl}
        alt={banner.title || ""}
        className="w-full object-cover max-h-[100px] md:max-h-[120px]"
        loading="lazy"
      />
    </div>
  );
