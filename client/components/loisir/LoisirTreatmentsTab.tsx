...
type LoisirTreatmentsTabProps = {
  treatments: LoisirTreatment[];
  onGoToSlots?: () => void;
  className?: string;
};

type CategoryId = "packs" | "buggy" | "quad" | "motocross" | "enfants" | "balades" | "options" | "autres";
...
type CategoryWithItems = CategoryDefinition & {
  label: string;
  items: LoisirTreatment[];
};

const CATEGORY_DEFINITIONS: CategoryDefinition[] = [
  { id: "packs", labelKey: "treatments.category.packs" },
  { id: "buggy", labelKey: "treatments.category.buggy" },
  { id: "quad", labelKey: "treatments.category.quad" },
  { id: "motocross", labelKey: "treatments.category.motocross" },
  { id: "enfants", labelKey: "treatments.category.kids" },
  { id: "balades", labelKey: "treatments.category.rides" },
  { id: "options", labelKey: "treatments.category.options" },
];
...
function guessCategoryFromLabel(label?: string): CategoryId | null {
  ...
  if (v.includes("buggy")) return "buggy";
  if (v.includes("motocross") || v.includes("moto")) return "motocross";

  if (v.includes("enfant") || v.includes("kids") || v.includes("junior")) return "enfants";

  if (v.includes("chameau") || v.includes("dromadaire") || v.includes("cheval") || v.includes("balade") || v.includes("randonnee")) {
    return "balades";
  }

  if (v.includes("option") || v.includes("transfert") || v.includes("gopro") || v.includes("photo") || v.includes("video")) {
    return "options";
  }

  if (v.includes("quad")) return "quad";

  return null;
}

function guessCategory(treatment: LoisirTreatment): CategoryId {
  ...
  if (title.includes("quad") && title.includes("enfant")) return "enfants";

  return "autres";
}

function sortTreatments(items: LoisirTreatment[]): LoisirTreatment[] {
...
function groupByCategory(treatments: LoisirTreatment[]) {
  const map: Record<CategoryId, LoisirTreatment[]> = {
    packs: [],
    buggy: [],
    quad: [],
    motocross: [],
...
function isPackLimited(pack: LoisirTreatment): boolean {
  const note = normalized(pack.note);
  const title = normalized(pack.title);
  return note.includes("limite") || title.includes("limite") || note.includes("offre") || title.includes("offre");
}

function PackCard({ pack, onReserve }: { pack: LoisirTreatment; onReserve?: () => void }) {
  const { t } = useI18n();
  ...
  return (
    <div className="rounded-2xl border border-primary/15 bg-primary/[0.04] p-4 sm:p-6">
      ...
      <div className="mt-4 flex items-end justify-between gap-4">
        <div className="text-sm text-slate-700">💰 {formatMad(pack.priceMad)} {t("common.per_person")}</div>
      </div>
...
function TreatmentRow({ treatment }: { treatment: LoisirTreatment }) {
...
export function LoisirTreatmentsTab({ treatments, onGoToSlots, className }: LoisirTreatmentsTabProps) {
  const { t } = useI18n();
  ...
  const categories: CategoryWithItems[] = React.useMemo(() => {
    const base = CATEGORY_DEFINITIONS.map((c) => ({ ...c, label: t(c.labelKey), items: grouped[c.id] }));
    return grouped.autres.length
      ? [...base, { id: "autres", labelKey: "treatments.category.other", label: t("treatments.category.other"), items: grouped.autres }]
      : base;
  }, [grouped, t]);
