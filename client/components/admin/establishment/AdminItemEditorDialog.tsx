 ...
 *
 * Version admin du dialogue d'édition d'item d'inventaire.
 * Réutilise la logique du ItemEditorDialog Pro mais avec les API admin.
 */
...
import {
  ...
  type SpicyLevel,
} from "@/components/pro/inventory/inventoryAllergens";
import { getLabelsForUniverse, labelById, normalizeLabels } from "@/components/pro/inventory/inventoryLabels";
import { centsToMoneyInput, parseMoneyInputToCents, toDatetimeLocalValue } from "@/components/pro/inventory/inventoryUtils";
...
async function adminUpdateItem(establishmentId: string, itemId: string, patch: Record<string, unknown>) {
  return adminApiFetch(`/establishments/${establishmentId}/inventory/items/${itemId}`, {
    method: "POST",
    body: JSON.stringify(patch),
  });
}

export function AdminItemEditorDialog({ open, onOpenChange, establishmentId, universe, categories, item, onSaved }: Props) {
  ...
  const canSubmit = useMemo(() => {
    ...
    return true;
  }, [saving, title, hasVariants, basePrice]);
  ...
  const submit = async () => {
    ...
    try {
      await adminUpdateItem(establishmentId, item.id, {
        title: title.trim(),
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex: Tajine d'agneau" />
            </div>

            <div className="space-y-2">
              <Label>Description <span className="text-xs text-slate-400 font-normal">(Français)</span></Label>
              <Textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="Description du produit..." />
            </div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {SPICY_LEVELS.map((level) => (
                        <button
                          ...
                          className={`
                            rounded-md border px-2 py-1 text-xs font-medium transition cursor-pointer
                            ${spicyLevel === level.id
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-4 space-y-3">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Button type="button" variant="outline" className="gap-2" onClick={addVariant}>
                    ...
                    {variants.map((v, idx) => (
                      <div key={idx} className="rounded-md border border-slate-200 p-3 space-y-3">
                        ...
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <Button type="button" variant="outline" onClick={() => removeVariant(idx)}>
