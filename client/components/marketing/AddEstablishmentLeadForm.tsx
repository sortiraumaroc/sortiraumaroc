...
export function AddEstablishmentLeadForm(props: {
  ...
  const baseInputCls = props.compact ? "h-10 rounded-xl text-sm" : "h-11 rounded-xl";
  const fieldGap = props.compact ? "space-y-1" : "space-y-1.5";
  ...
  return (
    <div className={props.compact ? "space-y-3" : "space-y-4"}>
      ...
      <div className={`grid grid-cols-1 sm:grid-cols-2 ${props.compact ? "gap-2" : "gap-3"}`}>
        <div className={fieldGap}>
      ...
      <Button
        ...
        onClick={() => void submit()}
        className={`w-full ${props.compact ? "h-10" : "h-12"} rounded-xl bg-primary text-white hover:bg-primary/90 font-extrabold transition-transform hover:scale-[1.01]`}
      >
