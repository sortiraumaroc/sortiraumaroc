...
import { uploadProInventoryImage, deleteProInventoryImage } from "@/lib/pro/api";
...
export function ImageUploader({
  ...
  const removePhoto = async (url: string) => {
    ...
    try {
      ...
      await deleteProInventoryImage({ establishmentId, url }).catch(() => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="text-xs text-slate-500 bg-slate-50 rounded-md p-2 space-y-1">
        ...
        <div>JPG, PNG, WebP, GIF • Max 5 MB par image • Résolution recommandée : 800×600 minimum</div>
      </div>
