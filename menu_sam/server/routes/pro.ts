...
import { ensureEstablishmentMenuSeeded } from "../lib/seed-establishment-menu";
...
proRouter.post("/bootstrap", async (_req, res) => {
  const email = process.env.PRO_BOOTSTRAP_EMAIL ?? "";
  const password = process.env.PRO_BOOTSTRAP_PASSWORD ?? "";
  const establishmentId = process.env.VITE_SAM_ESTABLISHMENT_ID ?? null;
  ...
  if (hasServerSupabaseServiceRole()) {
    ...
    if (existing) {
      ...
      if (establishmentId) {
        await ensureEstablishmentMenuSeeded(supabase, establishmentId);
      }
